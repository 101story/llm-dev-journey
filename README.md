...
| Phase 5 | LangGraph 에이전트 | 그래프 워크플로우, 조건 분기, 루프 | ✅ 완료 |
| Phase 6 | 실전 통합 프로젝트 | RAG + LangGraph + Qdrant + Confluence | ✅ 완료 |
---
...
│
└── phase6_rag_agent.py           # RAG + LangGraph + Qdrant 통합 에이전트
```
...
### Phase 6 — 실전 통합 프로젝트
Phase 2~5에서 배운 LangChain, RAG, Qdrant, LangGraph를 하나로 통합한
완전한 AI 에이전트를 구축합니다.

- `phase6_rag_agent.py` — 질문 분석 → RAG 검색 → 답변 생성 → 품질 평가 루프

**에이전트 노드 구성**:
- `analyze` — 질문이 문서 검색이 필요한지 LLM이 판단 (needs_rag 결정)
- `rag_search` — Qdrant에서 유사 문서 청크 검색 (날것의 문서 반환)
- `generate_rag` — 검색된 문서를 컨텍스트로 LLM이 답변 생성
- `generate_general` — 문서 무관 질문은 LLM이 직접 답변
- `evaluate` — 답변 품질 평가. 미흡하면 analyze로 재시도 (최대 3회)

**핵심 학습**: 검색(rag_search)과 생성(generate_rag)을 노드로 분리하면
각각 독립적으로 교체·개선이 가능합니다. LangGraph의 노드 함수는 state만
인자로 받아야 하므로 외부 객체(vectorstore) 주입 시 클로저를 활용합니다.
---
...
- LangGraph 노드 함수는 state 하나만 인자로 받아야 함 — 외부 객체 주입 시 클로저 또는 partial 활용
- rag_search(검색)와 generate_rag(생성)는 역할이 다름 — 검색은 날것의 문서 반환, 생성은 LLM이 정리
- Qdrant 영구 저장: `:memory:` → `QdrantClient(path="./qdrant_data")`로 교체하면 재실행 시 재인덱싱 불필요
